...
class TestSetup(FitnessFunctionsTestBase):
    def test_fitness_directory_created(self):
        self.assertFalse(
            os.path.exists(os.path.join(self.mock_project_path, "fitness"))
        )
    ...
    def test_database_created(self):
        self.assertFalse(
            os.path.exists(
                os.path.join(self.mock_project_path, "fitness", "fitness_metrics.db")
            )
        )
        run(self.mock_project_path, self.mock_code_path)
        self.assertTrue(
    ...
    def test_database_correct(self):
        ...
        result = connection.execute(
            """SELECT name FROM sqlite_master WHERE type='table' AND name='FITNESS_METRICS';"""
        )
